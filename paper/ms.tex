...
A series of related algorithms \cite{welch2009abc, wood2010sl, doucet2010pmcmc, ionides08, ionides15} enable parameter estimation within these models. In particular, the improved iterated filtering algorithm (IF2) of Ionides et al. \cite{ionides15} and the particle MCMC algorithm of Andrieu et al. \cite{doucet2010pmcmc} are the only simulation-based full-information maximum likelihood and fully Bayesian methods available in the literature thus far. However, accurate parameter estimation in the presence of significant Monte-Carlo noise in the likelihood estimate remains a significant challenge. Still, maximum likelihood parameter estimation can be challenging, especially when the Monte Carlo variance of the evaluation is high and the number of parameters is not small. For example, though in practice the algorithm of Ionides et al. \cite{ionides15} converges quickly to a neighborhood of the MLE, it often struggles to successfully optimize the last few-units of the log-likelihood. Ionides et al. \cite{Ionides_mcap} attempt to address the third problem, albeit with a computationally-expensive approach. 


\subsection{Automatic Differentiation for Particle Filters}

Recent advances in automatic differentiation (AD) for particle filters \cite{blei2018vsmc, jon2018diffpf, corenflos21, scibior2021dpf, doucet2022particlebased} have drawn attention to AD as an alternative tool for inference in partially observed stochastic processes with the particle filter. However, these existing approaches either are not yet compatible with the desire for simulation-based inference, or are computationally expensive. In particular, existing approaches are either asymptotically biased \cite{blei2018vsmc, jon2018diffpf}, have high variance \cite{poyiadjis11, scibior21}, computationally expensive \cite{corenflos21, chen24}, or require transition densities \cite{poyiadjis11, scibior21, doucet2022particlebased, chen24}.

In particular, the work of Scibior and Wood \cite{scibior2021dpf} demonstrates that the estimators derived in Poyiadjis et al. \cite{poyiadjis11} can be attained with AD, albeit with a simple tweak, that they call the stop-gradient trick, to the particle filter that does not modify its likelihood estimates. Though their method is not fully compatible with simulation-based inference and the mathematical motivation behind the stop-gradient trick is not apparent, we show that another simple tweak enables this compatibility and corresponds to a special case of our MOP-$\alpha$ algorithm. 

\subsection{Our Contributions}
We develop a new approach to statistical inference via AD for particle filters which addresses these concerns. We construct a new theoretical framework that addresses the seemingly incompatible paradox of differentiating through a Monte-Carlo algorithm with discontinuous resampling.

The key observation is that one can turn this into a problem of differentiating through a series of smooth measurement density ratios and (through the reparametrization trick) a differentiable simulator. This is done through the construction of a suitably reweighted family of particle filters that we call \textbf{Measurement Off-Policy-$\alpha$} (MOP-$\alpha$), for some $\alpha \in [0,1]$ that optimizes a bias-variance tradeoff. This family of algorithms encompasses the special case of the biased gradient estimator of \cite{blei2018vsmc} when $\alpha=0$, and the high-variance gradient estimate from \cite{poyiadjis11, scibior21} when $\alpha=1$, and does not require access to the transition densities -- only requiring access to a differentiable simulator.
...
We call $f_{X_n|X_{n-1}}\left(x_{n} \mid x_{n-1}; \theta\right)$ the process model, writing $\texttt{process}\left(x_n; \theta\right)$ for the simulator corresponding to the above process model, $f_{Y_n|X_n}\left(y_n \mid x_n; \theta\right)$ the measurement model, and again write $y_n^*$ for the actual values of the observations that were observed. As a matter of terminology, we call $f_{X_{1:N}|Y_{1:N}}(x_{1_N}|y_{1:N}^*; \theta)$ the posterior distribution of states, and $f_{X_{n}|Y_{1:n}}(x_n|y_{1:n}^*; \theta)$ the filtering distribution at time $t_n$. 
...
As mentioned earlier, instead of directly differentiating through a particle filter, one can instead differentiate through the simulator and a series of measurement density ratios. We construct an algorithm called \textbf{Measurement Off-Policy-$\alpha$} (MOP-$\alpha$) in Algorithm \ref{alg:mop} that targets the posterior and likelihood, and whose gradient yields the estimators of \cite{poyiadjis11, scibior21} and \cite{blei2018vsmc} as special cases. 

\subsection{Algorithm Outline} 
The broad idea is to run the particle filter twice under the same seed. We first run the particle filter under $\phi$, and then obtain a coupled set of particles by running a bootstrap filter with the process model at $\theta$ but the resampling indices from the previous run at $\phi$. We can then reweight the conditional likelihoods by a correction factor accumulated over time of the ratio of the measurement density under $\theta$ and the measurement density under $\phi$ to account for the ("off-policy") resampling under $\phi$. That is, when we write $g^{\theta}_{n,j}={f}_{{Y}_{n}|{X}_{n}}(y_{n}^{*}|{X}_{n,j}^{P,\theta}\giventh{\theta})$ as shorthand for the measurement density and estimate the conditional likelihood under $\phi$ in the usual way with $L_n^{\phi} = \frac{1}{J}\sum_{m=1}^{J}g^{\phi}_{n,m}$, we can estimate the conditional likelihood under $\theta$ as either
\begin{equation}
...
\end{equation}
where each particle is recursively weighted according to 
\begin{equation}
...
\end{equation}
and $k_j$ are the indices from resampling according to the measurement density at $\phi$, $k_j \sim \text{Categorical}(g^{\phi}_{n,1},...,g^{\phi}_{n,J})$. The before-resampling conditional likelihood estimate $L_n^{B,\theta,\alpha}$ should be used in general, as it has slightly lower variance than the after-resampling estimate $L_n^{A,\theta,\alpha} = L_n^\phi\cdot \frac{\sum_{j=1}^J w^{F,\theta}_{n,j}}{\sum_{j=1}^J  w^{P,\theta}_{n,j}}$, but the latter is useful in deriving properties of the MOP-$\alpha$ gradient estimate such as that in Theorem \ref{thm:mop-functional-forms}.
...
\begin{algorithm}[H]
    ...
    \label{alg:mop}
        ...
        \textbf{If $\theta \neq \phi$:}
            Run a particle filter once with parameters $\phi$ to obtain $X_{n,j}^{P,\phi}, X_{n,j}^{F,\phi}$, with seed $\omega$. \newline
        \textbf{Else:} Set $X_{n,j}^{P,\phi}, X_{n,j}^{F,\phi}$ to be copies of $X_{n,j}^{P,\theta}, X_{n,j}^{P,\theta}$ in the rest of the algorithm. \newline
		\textbf{Initialize } filter particles ${X}_{0,j}^{F,\theta}\sim {f}_{{X}_{0}}\left(\cdot\giventh{\theta}\right)$, relative weights $w^{F,\theta}_{0,j}= 1$ for $j$ in $\seq{1}{J}$. Fix $\omega.$ \newline
		\textbf{For} $n=1,...,N$: \newline
		...
		\textbf{Return:} likelihood estimate $\hat{\lik}(\theta) = \prod_{n=1}^N L_n^{A,\theta,\alpha}$ or $\hat{\lik}(\theta) = \prod_{n=1}^N L_n^{B,\theta,\alpha}$, filtering distribution $\{(X_{N,j}^{F, \theta}, w^{F,\theta}_{N,j})\}_{j=1}^J.$
\end{algorithm}


\subsection{Discounting Weights for a Bias-Variance Tradeoff}

As mentioned earlier, the correction factors accumulate over time. If $\theta$ and $\phi$ coincide, one can discount the correction factors from previous timesteps by some $\alpha \in [0,1]$, as seen in Equation \ref{eq:weighting-scheme}, while still targeting the posterior and likelihood at $\theta$. We do this in order to optimize a bias-variance tradeoff for the score estimate yielded by differentiating through the log-likelihood estimate provided by MOP-$\alpha$. When $\alpha=1$, MOP-$1$ maintains the memory of each particle's ancestral trajectory, recovering the consistent but high-variance gradient estimate from \cite{poyiadjis11, scibior21}. When $\alpha=0$, MOP-$0$ considers only single-step transition dynamics, recovering the low-variance but asymptotically biased gradient estimator of \cite{blei2018vsmc}. $\alpha$ therefore controls an exponentially-weighted moving average, determining how much "memory" the resulting gradient estimate has over ancestral trajectories. This is illustrated in Figure \ref{fig:biasvar}, in the context of the Dhaka cholera model of \cite{king08}. 
...
We show here that MOP-$\alpha$ targets the posterior and is strongly consistent for the likelihood. To do so, we state the definition of targeting and present three lemmas, whose proof can be found in the supplementary material.
\begin{defn}[Targeting]
    ...
    \end{equation}  
        A set of particles $(X^J_j, w^J_j), j=1,2, \ldots,J$, \textbf{targets} $\pi$ if for any measurable and bounded functional $h$, as $J\to\infty$
    \begin{equation}
...
From these lemmas, we can derive the following result.

\begin{thm}[MOP-$\alpha$ Targets the Posterior and Likelihood]
    \label{thm:mop-targeting}
    When $\alpha=1$ or $\phi=\theta$, MOP-$\alpha$ targets the posterior and is strongly consistent for the likelihood. That is, for $\pi=f_{X_{1:N}|Y_{1:N} ; \theta}$ and any measurable and bounded functional $h$ and for $\hat\lik(\theta) = \prod_{n=1}^N L_n^{A,\theta,\alpha}$ or $\prod_{n=1}^N L_n^{B,\theta,\alpha}$, it holds that
    \begin{equation}
        \frac{\sum_{j=1}^J h(x_{n,j}^{P, \theta}) w_{n,j}^{P,\theta}}{\sum_{j=1}^J w_{n,j}^{P,\theta}} \stackrel{a.s.}{\to} E_\pi h, \;\; \hat\lik(\theta)  \stackrel{a.s.}{\to} \lik(\theta).
    \end{equation}
\end{thm}

\begin{proof}
    When $\theta=\phi$, regardless of the value of $\alpha$, the ratio ${g_{n,j}^\theta}/{g_{n,j}^\phi}=1,$ and this reduces to the vanilla particle filter estimate.
    ...
    For the likelihood, if the likelihood is estimated with the before-resampling conditional likelihoods $\hat\lik(\theta) = \prod_{n=1}^N L_n^{B,\theta,\alpha}$, the strong consistency is a direct consequence of our earlier result that $\{ \big(X^{P,\theta}_{n,j},w^{P,\theta}_{n,j}\big) \}$ targets $f_{X_{n}|Y_{1:n-1}}(x_{n}|y^*_{1:n-1};\theta)$.  
...
\begin{thm}[MOP-$0$ and MOP-$1$ Functional Forms]
    ...
    \end{equation}
    yielding the gradient estimator of \cite{blei2018vsmc} with the bootstrap filter. When $\alpha=1$,
    \begin{equation}
        \nabla_\theta \hat{\ell}^1(\theta) 
        = \frac{1}{J}\sum_{j=1}^J \nabla_\theta \log f_{Y_{1:N}|X_{1:N}}\left(y_{1:N}^* | x_{1:n,j}^{A, F,\theta}\right),
    \end{equation}
    yielding the gradient estimator of \cite{poyiadjis11, scibior21} applied to the bootstrap filter.
\end{thm}

The proof follows from a useful decomposition of the after-resampling conditional likelihood estimate $L_n^{A,\theta,\alpha}$ that yields a telescoping product in the MOP-$1$ case. Repeated applications of the log-derivative identity that $\nabla_x \log(f(x)) = (\nabla_x f(x))/f(x)$, and noting that when $\theta=\phi$, that $w_{n,j}^{P,\theta}$ evaluates to $1$, yield the result. As such, we defer the proof to the supplementary material. 

This further illustrates the role of $\alpha$ as a paramteter that dictates the "memory" of the gradient estimate. As \cite{scibior2021dpf} remark, the MOP-$0$ estimator of \cite{blei2018vsmc} only considers single-step quantities, and is "memoryless" beyond a single step. This is in contrast to the case when $\alpha=1,$ as that estimate of \cite{poyiadjis11} only considers the surviving particles at time $N$, fully tracking dependencies over time. 


\subsection{MOP-$1$ Is Consistent for the Score}

Despite showing that the MOP-$1$ gradient estimate yields the estimate of \cite{poyiadjis11, scibior21} when applied to the bootstrap filter, \cite{poyiadjis11, scibior21} use $\frac{1}{J}\sum_{j=1}^J \nabla_\theta \log f_{Y_{1:N}, X_{1:N}}\left(y_{1:N}^* , x_{1:n,j}^{A, F,\theta}; \theta\right)$ in general instead of the bootstrap case of $\frac{1}{J}\sum_{j=1}^J \nabla_\theta \log f_{Y_{1:N}|X_{1:N}}\left(y_{1:N}^* | x_{1:n,j}^{A, F,\theta};\theta\right)$. These have different functional forms, and are not the same a-priori. As such we directly show the consistency of the MOP-$1$ gradient estimate below. 
...
One can think of MOP-$\alpha$ as taking a likelihood estimate at $\phi$, reweighting it to instead target the likelihood at $\theta$, and then taking the derivative with respect to $\theta$ of the above estimate. But if $\theta$ is evaluated at $\phi$, the particles at $\theta$ and $\phi$ coincide, and one only needs to run one particle filter at $\theta=\phi$, setting the particles at $\phi$ to be copies of the particles at $\theta$ where gradients don't propagate (and vice-versa for the measurement densities $g_{n,j}^\phi$ and conditional likelihood estimate $L_n^\phi$). This is done algorithmically through the \texttt{stop\_gradient()} function in \texttt{JAX}, and therefore provides a mathematical justification for the "stop-gradient trick" presented in \cite{scibior21}.

\subsection{Optimization}
This still leaves us with the question of designing an effective procedure for likelihood maximization with these new tools. We propose a simple algorithm we call Iterated Filtering with Automatic Differentiation (IFAD) in Algorithm \ref{alg:ifad} that amounts to warm-starting gradient descent that uses the gradient estimate given by MOP-$\alpha$ (or some other reasonable first or second order procedure) with the output of an initial search of IF2.

That is, a coarse solution is found by running IF2 with an aggressive random walk standard deviation (learning rate), until the search stalls to an initial "convergence" to what is hopefully a neighborhood of the MLE. We can then use gradient descent, or other first or second order gradient methods, to refine this coarse solution. We do this because IF2 has been empirically shown to converge quickly to a neighborhood of the MLE, and by switching to gradient descent with MOP-$\alpha$ estimates, one can bypass the difficulty that IF2 has with optimizing the last few units of log-likelihood. Conversely, gradient descent (and other first or second order gradient methods) can get stuck in saddle points and less desirable local minima when the likelihood is nonconvex. Warm-starting gradient methods with IF2 lets us get close enough to the MLE for gradient methods to shine, as in most cases regularity conditions ensure that the likelihood surface is well-behaved enough in this neighborhood that the aforementioned issues with saddle points and local minima are alleviated. Combining these two methods in this way lets us enjoy the best of both worlds.


\begin{algorithm}[H]
	\caption{Iterated Filtering with Automatic Differentiation}
    \label{alg:ifad}
	    \textbf{Input:} Number of particles $J$, timesteps $N$, measurement model $p(y_n^*|x_n, \theta)$, simulator $\process(x_{n+1}|x_n, \theta),$ IF2 cooling schedule $\eta_t$, MOP-$\alpha$ discounting parameter $\alpha$.\newline
		\textbf{Initialize} $\theta_0$, $t=0.$\newline
		...
		\hspace*{4mm} Obtain $g(\theta_t) = \nabla_{\theta_t} (-\loglik(\theta_t))$, $H(\theta_t)$ s.t. $\lambda_{\min}(H) \geq c$. \newline
...
We use the Dhaka cholera model of \cite{king08} to benchmark the performance of our method. This was also used or modified in \cite{ionides15, wood16, wycoff2024voronoi} for the same purpose. This is a stochastic SIR compartmental model with transition and measurement uncertainty, where the population at time $t$, $H_t$, is divided into the susceptible compartment $S_t$, infected compartment $I_t$, and three ($k=3$) recovered compartments $R^1_t, ..., R^k_t$ denoting varying degrees of cholera immunity. We write $M_t$ for the cholera deaths in each month. As in \cite{king08, ionides15}, the \textbf{transition dynamics} follow this series of stochastic differential equations:
\begin{align*}
    d S_t&=(k \epsilon R^k_t+\delta(S_t-H_t)-\lambda_t S_t) d t+d H_t-\frac{\sigma S_t I_t}{H_t} d B_t, \\[-2mm] 
    d I_t&=\left(\lambda_t S_t-(m+\delta+\gamma) I_t\right) d t+\frac{\sigma S_t I_t}{H_t} d B, \\[-2mm]
    d R^1_t&=\left(\gamma I-(k \epsilon+\delta) R^1\right) d t, \\ 
    d R^k_t&=\left(k \epsilon R^{k-1}_t-(k \epsilon+\delta) R^k_t\right) d t,
\end{align*}

where $B_t$ represents Brownian motion, $m$ is the rate at which infected individuals succumb to cholera, $\gamma$ is the rate at which infected individuals recover, $1/\epsilon$ is the mean duration of immunity, $\sigma$ represents the standard deviation of perturbations to the force of infection, and $\delta=0.02$ is the death rate. 

The \textbf{force of infection}, $\lambda_t$, is modeled by splines $(s_j)_{j=1}^6$
\begin{equation}
    \lambda_t=\exp\hspace{-1mm}\left(\hspace{-.5mm}\beta_{\text{trend}}(t-t_0)+\hspace{-1mm}\sum_{j=1}^{6} \beta_j s_j(t)\hspace{-1mm}\right)\hspace{-1mm}\frac{I_t}{P_t} + \exp \left(\sum_{j=1}^{6} \omega_j s_j(t)\hspace{-1mm}\right)\hspace{-1mm},
\end{equation}
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=\textwidth/5]{../imgs/095/pairs.png}
    \includegraphics[width=\textwidth/5]{../imgs/095/qq.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth/4]{../imgs/095/boxplot.png}
    \includegraphics[width=\textwidth/5]{../imgs/095/optim.png}
...
\end{figure}
\kevin{Change the optim graph to include median search and best search with dashes as linestyle, remove the shaded areas. }
