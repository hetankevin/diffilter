...
\begin{abstract}
  Automatic differentiation (AD) has driven recent advances in machine learning, including deep neural networks and Hamiltonian Markov Chain Monte Carlo methods. Partially observed nonlinear stochastic dynamical systems have proved resistant to AD techniques because widely used particle filter algorithms yield an estimated likelihood function that is discontinuous as a function of the model parameters.  We show how to embed two existing AD particle filter methods in a theoretical framework that provides an extension to a new class of algorithms within which we obtain a favorable bias/variance tradeoff. Further, we develop likelihood maximization algorithms suited to the Monte Carlo properties of the derivative estimate. Our algorithms require only a differentiable simulator for the latent dynamic system; by contrast, most previous approaches to AD likelihood maximization for particle filters require access to the system's transition probabilities, and this can be prohibitive. Promising numerical results indicate that a hybrid algorithm that uses AD to refine a coarse solution from an iterated filtering algorithm can beat current state-of-the-art methods on a challenging scientific benchmark problem.
\end{abstract}
...
\dropcap{M}any scientific models involve highly nonlinear stochastic dynamic systems possessing significant random variation in both the process dynamics and the measurements.
...
POMP models arise in fields as diverse as epidemiology \cite{he10, stocks17}, ecology \cite{knape12}, and finance \cite{kim08, breto14}.
...
We suppose that the discete-time latent process model has a density $f_{X_n|X_{n-1}}\left(x_{n} \mid x_{n-1}; \theta\right)$, and we write $\texttt{process}\left(x_n\mid x_{n-1}; \theta\right)$ for its corresponding simulator \ed{WE DON'T USE THIS... MAYBE CHANGE HERE, OR USE IT}.
...
Superscripts $x_{n,j}^A$ denote the ancestral trajectory of particle $j$ at time $n$, and $a(\cdot)$ is the ancestor function \ed{NOT DEFINED YET. THIS WILL PUZZLE READERS UNLESS THEY ARE ALREADY HIGHLY FAMILIAR WITH THE TOPIC}.
For simplicity, we omit the dependence of the likelihood and other suitable calculations on $X_0$. \ed{ALGORITHM 1 HAS $f_{X_0}$. IN GENERAL, THIS DETAIL IS BEST INCLUDED. INITIAL VALUES CAN BE AN IMPORTANT PART OF A DYNAMIC SYSTEM.}
...
\ed{THE SEED CORRESPONDS TO ALL THE MONTE CARLO QUANTITIES GENERATED, INCLUDING THINGS THAT HAVE NO DIRECT ROLE IN THE MODEL, SO MAYBE THIS EXPLANATION NEEDS ADDITIONAL CARE.}
...
\begin{algorithm}[H]
    ...
    \label{alg:mop}
		...
		\textbf{For} $n=1,...,N$: \newline
            ...
            \hspace*{4mm} Resample particles ${X}_{n,j}^{F,\phi}={X}_{n,k_{j}}^{P,\phi}$. \ed{$\phi$ SHOULD BE $\theta$} \newline
...
\ed{I REWROTE THIS TO BE MORE EXPLICIT, BUT MAYBE THIS NEEDS MORE EDITING. IT MAKES ME THINK THAT ``RESAMPLING OFF-POLICY'' WOULD BE A BETTER NAME!}
...
We then reweight the conditional likelihoods by a correction factor accumulated over time to account for the resampling under $\phi$. That is, writing $g^{\theta}_{n,j}={f}_{{Y}_{n}|{X}_{n}}(y_{n}^{*}|{X}_{n,j}^{P,\theta}\giventh{\theta})$ for the measurement density, $L_n^{\phi} = \frac{1}{J}\sum_{m=1}^{J}g^{\phi}_{n,m}$ for the conditional likelihood estimate under $\phi$,  we can estimate the conditional likelihood under $\theta$ as
\vspace*{-2.5mm}
\begin{equation}
...
One can discount the correction factors from previous timesteps by some $\alpha \in [0,1]$, as seen in Equation \ref{eq:weighting-scheme}, while still targeting the posterior and likelihood if $\theta=\phi$.
...
We now consider the question of designing an effective procedure for likelihood maximization with MOP-$\alpha$.
We propose a simple algorithm we call Iterated Filtering with Automatic Differentiation (IFAD) in Algorithm \ref{alg:ifad} that runs some preliminary iterations of IF2 to warm-start an iterative first or second-order method that uses the MOP-$\alpha$ gradient estimate.
This leverages IF2's empirically stable and rapid convergence to a neighborhood of the MLE, overcoming the tendency of gradient methods to get stuck in saddle points and local minima.
Conversely, switching to gradient ascent with MOP-$\alpha$ score estimates lets one bypass the difficulty experienced with IF2 when optimizing the last few units of log-likelihood.
Combining these two methods lets us enjoy the best properties of both algorithms, which we show leads to strong empirical performance.
...
\ed{IN IFAD, $t$ IS USED AS AN INDEX FOR ITERATIONS. PREVIOUSLY, WE HAVE USED $m$ FOR THIS SINCE $n$ AND $t$ ARE USED ELSEWHERE.}

\ed{IFAD HERE IS A 2ND ORDER ALGORITHM. MAYBE FOCUS ON THE PRIMARY EXAMPLE, WHICH IS FIRST ORDER. ALSO, THE STOPPING RULE SHOULD DESCRIBE WHAT WAS DONE IN THE EXEMPLES}
...
\begin{algorithm}[H]
    ...
    \label{alg:ifad}
        ...
        \textbf{Preliminary iterated filtering:} Run IF2 under cooling schedule $\eta_t$ to obtain $\{\Theta_j, j=1,...,J\}$, set $\theta_t := \frac{1}{J}\sum_{j=1}^J \Theta_j.$\newline
...
We write $H(t)$ for the total population, and $M_t$ \ed{$M_n$ ??} for the cholera deaths in each month.
As in \cite{king08, ionides15}, the transition dynamics follow a series of stochastic differential equations:
\ed{CHANGE TO $S(t)$ etc FOR CONSISTENCY.}
\vspace*{-1mm}
\begin{align*}
    ...
    \vspace*{-2mm}
\end{align*}
with Brownian motion $B_t$, cholera death rate $m$, recovery rate $\gamma$, mean immunity duration $1/\epsilon$, standard deviation of the force of infection $\sigma$, and population death rate $\delta=0.02$. The force of infection, $\lambda_t$, is modeled by splines $(s_j)_{j=1}^6$
\vspace*{-2mm}
\begin{equation*}
...
\paragraph{IFAD Successfully Finds the MLE:} Previously, the MLE at a log-likelihood of $-3748.5$ was found by \cite{king16} with much effort, using many global and local IF2 searches, with the assistance of likelihood profiling. Meanwhile, \cite{ionides15} only achieve a maximum log-likelihood of $-3768.63$, while the best log-likelihood found by \cite{king08} was only $-3793.4$. Despite being initialized for a global search, IFAD manages to find the MLE over the 100 searches, as seen in Table \ref{table:mle}. This suggests that the sequence of local searches and refinement that was previously necessary for finding the MLE with IF2 is not necessary with the IFAD algorithm. 
...
\begin{enumerate}[label=(A\arabic*),itemsep=-1.2ex] 
    ...
    \item \textbf{Bounded Measurement Model.} There exist $\underbar{G}, \bar{G}$ such that $0<\underbar{G} \leq f_{Y_n \mid X_n}\left(y_n^* \mid x_n; \theta\right) \leq \bar{G}<\infty$ and there exists $G'(\theta)$ with $||\nabla_\theta \log f_{Y_n \mid X_n}\left(y_n^* \mid x_n; \theta\right)||_\infty \leq G'(\theta)< \infty$. \label{assump:bounded-measurement} \ed{I EDITED THE $G'(\theta)$ BIT. CHECK THAT IT IS RIGHT. IT STILL SEEMS A BIT UNCLEAR TO ME. ALSO, IT HAS SOME RELATIONSHIP TO A4 WHICH IS UNCLEAR TO ME.}
...
Despite showing that the MOP-$1$ gradient estimate yields the estimate of \cite{poyiadjis11, scibior21} when applied to the bootstrap filter, \cite{poyiadjis11, scibior21} use $\frac{1}{J}\sum_{j=1}^J \nabla_\theta \log f_{Y_{1:N}, X_{1:N}}\left(y_{1:N}^* , x_{1:n,j}^{A, F,\theta}; \theta\right)$ in general. The bootstrap and general estimates have different functional forms, and are not the same a-priori \ed{WHAT DOES THIS MEAN? THAT IT IS NOT IMMEDIATLY CLEAR THEY ARE THE SAME?}. As such we directly show the consistency of the MOP-$1$ gradient estimate below. 
...
\begin{thm}
    ...
    When $\alpha\in(0,1)$, $\theta=\phi$, the MSE and variance of MOP-$\alpha$, where $\psi(\alpha)=(\alpha^k  + \alpha^{k+1} - \alpha)(1-\alpha)^{-1}$ and $\epsilon$ depends on $\bar{M}, \underbar{M}, \bar{G}, \underbar{G}$ as in \cite{karjalainen23} are:
    \vspace*{-1ex}
    \begin{align*}
...
Each iteration of IF2 amounts to one iteration of the particle filter, with $O(NJ)$ time complexity. The initial convergence to a region of high likelihood happens fairly quickly in practice. In the case of the Dhaka cholera model of \cite{king08}, when a geometric cooling multiplier of 0.95 and initial random walk standard deviation of 0.02 is used, initial convergence happens within 40 iterations of IF2. In comparison, often 100 or 200 iterations of IF2 are used for an initial global search. \cite{ionides15}, for example, use 100 iterations with the Dhaka cholera model of \cite{king08}, while \cite{wheeler23} use 200 with Model 1 in \cite{lee20}.

In line with the cheap gradient principle of \cite{kakade2019provably}, getting a gradient estimate from MOP-$\alpha$ takes no more than 6 times that of the runtime of the particle filter. In practice, the multiplier is 3.75.
AD for MOP-$\alpha$ therefore also has $O(NJ)$ time complexity, obtaining a gradient estimate with runtime linear in the number of particles.

\subsection{Python Library}

Implementing the particle filter, simulator, and MOP-$\alpha$ in \texttt{JAX} \cite{jax} enabled us to take advantage of just-in-time compilation and GPU acceleration, with a simulator written in Python. This led to a 16x speedup (379ms vs 6.29s on a Intel i9-13900K CPU and NVIDIA RTX3090 GPU) over the CPU-only implementation of the particle filter in the \texttt{pomp} package of \cite{king16}, for which the simulator is written in C and called from R.
...
We note that it is possible to work without a differentiable simulator.
This arises if the latent process takes values in a discrete set.
In this case, one could extend MOP-$\alpha$ to have an off-policy process model as well as off-policy resampling.
This requires access to ratios of transition densities, so it does not have the plug-and-play property.
