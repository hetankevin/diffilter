...
We will now use this result to bound $\text{Var}(\nabla_\theta \hat s_k(\theta)$ proper. For random variables $X,Y$ where $|X-Y|$ is bounded almost surely by some $M$, we have that \kevin{I proved this on paper in the office, need to write the proof down in a later section.}
...
\begin{align*}
    \text{Var}(\nabla_\theta \hat\ell(\theta)) 
    &\lesssim \min_k\left(\frac{c_p^2k^2G'(\theta)^2N}{J} + \frac{4\alpha^{2k}}{(1-\alpha)^2}N^2G'(\theta)^2 \right)\\
...
\begin{align*}
    ...
    &\leq \frac{1}{J}\sum_{j=1}^J \sum_{i=1}^n\left(1-\alpha^{(n-i)}\right)\left\lVert\nabla_\theta\log\left(g_{i,j}^{A,F,\theta} \right)\right\rVert_{\infty}\\
    &\leq \frac{1}{J}\sum_{j=1}^J \sum_{i=1}^n\left(1-\alpha^{(n-i)}\right)G'(\theta)\\
