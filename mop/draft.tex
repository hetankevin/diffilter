...
\begin{aspt}[Bounded Process Model]
...
\begin{aspt}[Bounded Measurement Model]
...
\begin{aspt}[Locally Bounded Derivatives]
...
\begin{aspt}[Bounded And Differentiable Measurement Density Ratios]
...
\begin{aspt}[Bounded And Differentiable Process Model]
...
\begin{prop}[Smooth Neighborhood]
...
\kevin{Take derivative of loglik with regard to alpha?}
\kevin{Talk about regime change the moment you move away from alpha=1, variance going from N2 to N, and bias going from 0 to something, but try to prove that decrease in variance outweights increase in bias. }
\kevin{Create tools for choosing alpha via binary search CV from an averaged alpha=1 target}
\kevin{suppose at each filtering step, look at gradient calculated at alpha=0, dynamically select alpha during filtering. calculate bias and variance online during filtering, and update alpha accordingly. }


\kevin{No need to recompute so many particle filters! Just take an array of weight copies, one for each alpha. Gradients for each alpha come free the moment you compute one of them. }

\kevin{Elbow method on variance, since bias is hard to compute? }
\kevin{Get variance estimate, bias estimate, construct confidence intervals on bias estimate, use that to get unbiased estimate of the MSE. }
...
\kevin{I think we can establish the linear convergence for the entire procedure in the same sense as \href{this}{https://arxiv.org/pdf/2311.08442.pdf}, in the sense that if IF2 is run for a sufficiently large but fixed number of iterations, once we arrive in a local strongly convex region, we get linear convergence when we swap to MOP. Show by induction that we remain in the strongly convex region (in terms of parameter space and likelihood), eqn 218. }
...
\kevin{Reach: PNAS. Else try for JRSSB, try to get Chopin for Annals AE. Would need to change bracketing conventions for Biometrika. If submitting to PNAS, JASA, Biometrika, or JRSSB, highlight the different statistical gradient estimator and finite sample performance, rewrite introduction, maybe create a way to use the gradients for inference rather than using profiling. Try to rewrite it as a breakthrough in statistics. Safety: JMLR}
...
\kevin{I have no clue how to prove this. Help!}
\begin{conj}[MOP-1 Estimate Is Unbiased for the Likelihood and the Gradient of the Likelihood]
...
%     Observe that our differentiable simulator $\process(\cdot, \theta)$ and choice of weights now satisfy:

%     - A Feynman-Kac model $\left\{M_t, G_t\right\}$ such that:
    
% - the weight function $G_t$ may be evaluated pointwise (for all $t$ );

% - it is possible to simulate from $\mathbb{M}_0\left(\mathrm{~d} x_0\right)$ and from $M_t\left(x_{t-1}, \mathrm{~d} x_t\right)$ (for any $x_{t-1}$ and $t$ ).

% - The number of particles $N$.

% - The choice of an unbiased resampling scheme (e.g. multinomial, residual, stratified, systematic), that is, an algorithm to generate variables $A_t^{1: N}$ in $1: N$ such that $\mathbb{E}\left[\sum_{m=1}^N \mathbbm{1}\left\{A_t^m=n\right\}\right]=N W_{t-1}^n$; see previous chapter for more details.
\end{proof}


\begin{lem}[Consistency of MOP-1 Gradient Estimate]
...
\begin{proof}
    ...
    % We know that the variance of the MOP-1 gradient estimate $\nabla_\theta \hat\ell_J(\theta) = \frac{1}{J}\sum_{j=1}^J \nabla_\theta w_{N,j}^{F,\theta}$ goes to zero coordinate-wise as $J\to\infty$ by the particle filter law of large numbers for bounded functionals of particles (\kevin{Prove general LLN for functionals of MOP under boundedness assumptions in L2, but maybe Hoeffding may be enough to show this, because we only want convergence in probability, we just need to show the expectation converges}), so it suffices to show that it is asymptotically unbiased.

    % Write $\hat\pi_{n,J}$ for the filtering distribution at timestep $n$ and with $J$ particles, $\hat\pi_{1:N,J}$ for the sequence of filtering distributions from timesteps $1$ to $N$, and $\pi_n, \pi_{1:N}$ for the respective posteriors. 

    % From Lemma \ref{lem:lik-proper-weight}, we know that $f_{Y_n|Y_{1:n-1}}(y_n^*|y_{1:n-1}^*, \theta)$ is consistently estimated by the conditional likelihood estimate obtained by MOP-1, for all $\theta$ in an open neighborhood of $\phi$. The likelihood is simply the product of the conditional likelihoods, and similarly for the estimates, so the likelihood $\lik(\theta)$ is also consistently estimated by the likelihood estimate obtained by MOP-1, $\hat\lik_J(\theta)$, for all $\theta$ in an open neighborhood of $\phi$, $\gN(\phi)$.

    % We therefore have that 
    % $$\lim_{J\to\infty} \E[\hat\lik_J(\theta)] = \lik(\theta)$$
    % pointwise for all $\theta \in \gN(\phi)$. Given that the gradient is bounded uniformly for all $J$ and consistency for the likelihood estimate holds for all $\theta$ in an open neighborhood of $\phi$ (and therefore all $\theta'$ in a sufficiently small neighborhood of $\theta$), the bounded convergence theorem allows us to switch the expectation and derivative:
    % $$\nabla_\theta \E[\hat\lik_J(\theta)] 
    % = \E[\nabla_\theta \hat\lik_J(\theta)].$$ 
    
    % Take limits on both sides to find that 
    % $$\lim_{J \to \infty} \nabla_\theta \E[\hat\lik_J(\theta)] 
    % = \lim_{J \to \infty} \E[\nabla_\theta \hat\lik_J(\theta)].$$ 

    % We then need to swap the limit and derivative on the left-hand side. This requires uniform convergence of $\lim_{J \to \infty} \nabla_\theta \E[\hat\lik_J(\theta)]$, i.e. that $$\sup_{\theta \in \gN(\phi)} ||  \nabla_\theta \E[\hat\lik_J(\theta)] - \lim_{J \to \infty} \nabla_\theta \E[\hat\lik_J(\theta)]||_{\infty} \to 0 \text{ as } J \to \infty,$$
    % or that $\forall \epsilon>0$ $\exists N\in \mathbb{N}$ such that $n>N => ||  \nabla_\theta \E[\hat\lik_J(\theta)] - \lim_{J \to \infty} \nabla_\theta \E[\hat\lik_J(\theta)]||_{\infty} < \epsilon$ $\forall \theta \in \gN(\phi).$ However, it is easier to instead use an indirect argument. 

    % We then need to swap the limit and derivative on the left-hand side. This requires uniform convergence. 
    ...
    \kevin{Our previous understanding was that the target is differentiable, and the sequence is differentiable in increasingly small open neighborhoods. This may not be true. The neighborhoods the sequence is differentiable in should be independent of $J$, as the resampling does not change with $\theta$ as it is dependent on $\phi$. Either way, the target is in each of the neighborhoods, and this may be sufficient in itself.} 
    ...
    % Taking expectations on both sides and using the bounded convergence theorem to interchange limits and expectations, we see that
    % $$\lim_{J \to \infty} \nabla_\theta \E[\hat\lik_J(\theta)] = \E[\lim_{J \to \infty} \nabla_\theta \hat\lik_J(\theta)] = \E[\nabla_\theta \lim_{J \to \infty} \hat\lik_J(\theta)] = \nabla_\theta \lim_{J \to \infty} \E[\hat\lik_J(\theta)].$$
    ...
    % We then have mean-square consistency for the gradient of the likelihood estimate. The result then follows by the continuous mapping theorem. 

    \kevin{Explain that the difference in formula between this and Poyiadjis is due to us taking the derivative w.r.t. the particles in the chain rule. }

    \kevin{Alternative argument: Show gradient is unbiased, after showing the likelihood is unbiased in an open neighborhood of phi. That should hold by Chopin's argument for unbiased resampling to nonconstant weights. Gradient unbiased if can swap expectation and derivative, I think with two continuous derivatives and boundedness, might need BCT/DCT. No need to swap limit and derivative in this case, or worrying about uniform limits.}
...
\kevin{Show bias is order N, variance is order N when alpha below 1. That implies optimal choice of alpha is not 1. }
...
\kevin{https://math.stackexchange.com/questions/3274996/is-a-transformed-mixing-process-is-still-mixing}

%$$\text{Cov}(x_{n,1:J}^P, x_{m, 1:J}^P) \leq (1-\epsilon)^{\floor{\frac{|n-m|}{c\log(J)}}}$$

\kevin{Ed: Claim 11.4.2, Chopin, is the key to reasoning about this. Establish geometric mixing, that $\text{Cov}(\nabla_\theta\hat\ell_n, \nabla_\theta\hat\ell_m) \leq \rho^{|n-m|}V_n$, where $V_n$ is asymptotically stable (the latter only holds for $\alpha<1$, as only then do we get asymptotic stability, otherwise it increases linearly in $n$). Geometrically decreasing covariances and stable variances over time are the same thing. Then, we can bound the total variance by $\text{Var}(\sum_n \nabla_\theta \hat\ell_n(\theta)) = \sum_{m,n} \text{Cov}(\nabla_\theta\hat\ell_n(\theta), \nabla_\theta\hat\ell_m(\theta)) = NV_n + 2(N-1)\rho V_n + 2(N-3)\rho^2V_n + ...$}
...
% \begin{align*}
%     &\frac{16\alpha^{2k}}{(1-\alpha)^2}N^2G'(\theta)^2 \leq 1,\\
%     &\alpha^{2k} \leq \frac{(1-\alpha)^2}{16N^2G'(\theta)^2},\\
%     &k \geq \frac{\log\left(16N^2G'(\theta)^2\right)-2\log(1-\alpha)}{2\log\left(\frac{1}{\alpha}\right)}.
% \end{align*}

% To interpret this result, if there exists some $k$ such that $$N \geq k \geq \frac{\log\left(16NG'(\theta)\right)-\log(1-\alpha)}{\log\left(\frac{1}{\alpha}\right)},$$
% we can choose such a $k$, as well as $k=N$, to see that
% \begin{align*}
%     \text{Var}(\nabla_\theta \hat\ell(\theta)) 
%     &\lesssim \min_{k\leq N}\left(\frac{c_d^2k^2r^2G'(\theta)^2N}{J} + \frac{\alpha^{2k}}{(1-\alpha)^2}N^2G'(\theta)^2 \right)\\
%     &\lesssim \min\left\{1+\frac{c_d^2G'(\theta)^2N}{J}, \frac{c_d^2G'(\theta)^2r^2N^3}{J}\right\} \\
%     &\leq \min\left\{1+\frac{c_d^2G'(\theta)^2N}{J}, \frac{c_d^2G(\theta)^2r^2N}{J}\right\},
% \end{align*}
...
\begin{align*}
    ...
    &= \left\lVert\sum_{n=1}^N \nabla_\theta \log\left(\frac{\sum_{j=1}^J\prod_{i=1}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)}{\sum_{j=1}^J\prod_{i=1}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)}\right) - \sum_{n=1}^N \nabla_\theta\log\left(\frac{\sum_{j=1}^J\prod_{i=1}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}}{\sum_{j=1}^J\prod_{i=1}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}}\right) \right\rVert_\infty\\
    ...
    &\qquad\qquad -\log\left(\sum_{j=1}^J\prod_{i=1}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}\right) + \log\left(\sum_{j=1}^J\prod_{i=1}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}\right)\Bigg)\Bigg\rVert_\infty \\
...
\kevin{This subsection is not useless I think! It shows that the bias vanishes continuously as $\alpha$ goes to $1$.}


\subsection{An Improved Bound by Mixing}

We decompose the bias $||\nabla_\theta\ell(\theta) - \E\nabla_\theta \hat\ell^\alpha(\theta)||_\infty \leq \E||\nabla_\theta\ell(\theta) - \nabla_\theta \hat\ell^\alpha(\theta)||_\infty$ into
$$\E||\nabla_\theta\ell(\theta) - \nabla_\theta \hat\ell^\alpha(\theta)||_\infty \leq \E||\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)||_\infty + \E||\nabla_\theta\hat s_k^1(\theta) - \nabla_\theta\hat s_k^\alpha(\theta)||_\infty + \E||\nabla_\theta\hat s_k^\alpha(\theta) -  \nabla_\theta\hat\ell^\alpha(\theta)||_\infty.$$
...
\begin{align*}
    &\E||\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)||_\infty \\
    &\leq \sum_{n=1}^N \E||\nabla_\theta\ell_n(\theta) - \nabla_\theta\hat s_{n,k}^1(\theta)||_\infty \\
    &\leq \sum_{n=1}^N \E||\nabla_\theta\ell_n(\theta) - \nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta)||_\infty + \sum_{n=1}^N \E||\nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta) - \nabla_\theta\hat s_{n,k}^1(\theta)||_\infty.
\end{align*}
...
$$\E||\nabla_\theta\ell_n(\theta) - \nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta)||_\infty \leq \frac{CG'(\theta)k}{\sqrt{J}}.$$
...
Then, we can bound 
$\E||\nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta) - \nabla_\theta\hat s_{n,k}^1(\theta)||_\infty$ by
...
\begin{align*}
    \E||\nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta) - \nabla_\theta\hat s_{n,k}^1(\theta)||_\infty 
    &\lesssim \sup_{\mu, \nu} \sup_{||\psi||_\infty \leq 1/2} G'(\theta)\left|(\mu \textbf{M}_{n,n+k})(\psi)-(\nu \textbf{M}_{n,n+k})(\psi)\right| \\
    &\leq \sup_{\mu, \nu} G'(\theta)\|\mu \textbf{M}_{n,n+k}-\nu \textbf{M}_{n,n+k}\|_{\mathrm{TV}} \\
    &\leq G'(\theta)(1-\epsilon)^{\floor{k/(c\log(J))}} ||\hat\pi_{n-k} - \pi_{n-k}||_{\text{TV}} \\
    &\leq G'(\theta)(1-\epsilon)^{\floor{k/(c\log(J))}} \sup_{||\psi||_{\infty} \leq 1/2} |\hat\pi_{n-k}(\psi) - \pi_{n-k}(\psi)| \\
    &\lesssim G'(\theta)(1-\epsilon)^{\floor{k/(c\log(J))}}.
\end{align*}
...
\begin{align*}
    &\E||\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)||_\infty \\
    &\leq \sum_{n=1}^N \E||\nabla_\theta\ell_n(\theta) - \nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta)||_\infty + \sum_{n=1}^N \E||\nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta) - \nabla_\theta\hat s_{n,k}^1(\theta)||_\infty\\
    &\lesssim N\frac{CG'(\theta)k}{\sqrt{J}} + NG'(\theta)(1-\epsilon)^{\floor{k/(c\log(J))}}.
\end{align*}


Now that the first term $\E||\nabla_\theta\ell(\theta)$ is taken care of, it remains to bound $$\E||\nabla_\theta\hat s_k^1(\theta) - \nabla_\theta\hat s_k^\alpha(\theta)||_\infty \text{ and } \E||\nabla_\theta\hat s_k^\alpha(\theta) -  \nabla_\theta\hat\ell^\alpha(\theta)||_\infty.$$


We see, by a similar argument to the variance bound, that
\begin{align*}
    &\E||\nabla_\theta\hat s_k^1(\theta) - \nabla_\theta\hat s_k^\alpha(\theta)||_\infty\\
    &= \E\left\lVert\sum_{n=1}^N \nabla_\theta \log\left(\frac{\sum_{j=1}^J\prod_{i=n-k}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)}{\sum_{j=1}^J\prod_{i=n-k}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)}\right) - \sum_{n=1}^N \nabla_\theta\log\left(\frac{\sum_{j=1}^J\prod_{i=n-k}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}}{\sum_{j=1}^J\prod_{i=n-k}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}}\right) \right\rVert_\infty\\
    &= \E\Bigg\lVert\sum_{n=1}^N \nabla_\theta \Bigg(\log\left(\sum_{j=1}^J\prod_{i=n-k}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)\right)- \log\left(\sum_{j=1}^J\prod_{i=n-k}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)\right)\\
    &\qquad\qquad -\log\left(\sum_{j=1}^J\prod_{i=n-k}^n\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}\right) + \log\left(\sum_{j=1}^J\prod_{i=n-k}^{n-1}\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)^{\alpha^{(n-i)}}\right)\Bigg)\Bigg\rVert_\infty \\
    &= \E\left\lVert\sum_{n=1}^N \nabla_\theta \Bigg(\log\left(\sum_{j=1}^Jw_{n,j}^{F,\theta,1,k}\right)- \log\left(\sum_{j=1}^Jw_{n,j}^{F,\theta,\alpha,k}\right)
    -\log\left(\sum_{j=1}^Jw_{n-1,j}^{A, F,\theta,1,k}\right) + \log\left(\sum_{j=1}^Jw_{n-1,j}^{A, F,\theta,\alpha,k}\right)\Bigg)\right\rVert_\infty \\
    ...
    +\left|\log\left(\sum_{j=1}^Jw_{n-1,j}^{A, F,\theta,1,k}\right) + \log\left(\sum_{j=1}^Jw_{n-1,j}^{A, F,\theta,\alpha,k}\right)\right|\Bigg)\right\rVert_\infty.
\end{align*}
...
\begin{align*}
    &\left\lVert\nabla_\theta\log\left(\sum_{j=1}^J w_{n,j}^{F,\theta,1,k}\right)-\nabla_\theta\log\left(\sum_{j=1}^J w_{n,j}^{F,\theta,\alpha,k}\right)\right\rVert_{\infty}\\
    &= \left\lVert\frac{\nabla_\theta\sum_{j=1}^J w_{n,j}^{F,\theta,1,k}}{{\sum_{j=1}^J w_{n,j}^{F,\theta,1,k}}}-\frac{\nabla_\theta\sum_{j=1}^J w_{n,j}^{F,\theta,\alpha,k}}{{\sum_{j=1}^J w_{n,j}^{F,\theta,\alpha,k}}}\right\rVert_{\infty} \\
    &= \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta w_{n,j}^{F,\theta,1,k}-\frac{1}{J}\sum_{j=1}^J \nabla_\theta w_{n,j}^{F,\theta,\alpha,k}\right\rVert_{\infty} \\
    &= \left\lVert\frac{1}{J}\sum_{j=1}^J \frac{\nabla_\theta w_{n,j}^{F,\theta,1,k}}{w_{n,j}^{F,\theta,1,k}}-\frac{1}{J}\sum_{j=1}^J \frac{\nabla_\theta w_{n,j}^{F,\theta,\alpha,k}}{w_{n,j}^{F,\theta,\alpha,k}}\right\rVert_{\infty} \\
    &= \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta \log\left(w_{n,j}^{F,\theta,1,k}\right)-\frac{1}{J}\sum_{j=1}^J \nabla_\theta \log\left(w_{n,j}^{F,\theta,\alpha,k}\right)\right\rVert_{\infty} \\
    &= \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta \left(\log\left(w_{n,j}^{F,\theta,1,k}\right)-\log\left(w_{n,j}^{F,\theta,\alpha,k}\right)\right)\right\rVert_{\infty}\\
    &= \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta \left(\sum_{i=n-k}^n\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right) - \sum_{i=n-k}^n{\alpha^{(n-i)}}\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)\right)\right\rVert_{\infty} \\
    &= \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta \left(\sum_{i=n-k}^{n}(1-\alpha^{(n-i)})\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right) \right)\right\rVert_{\infty}\\
    &\leq \frac{1}{J}\sum_{j=1}^J \sum_{i=n-k}^{n}(1-\alpha^{(n-i)})\left\lVert\nabla_\theta\log\left(g_{i,j}^{A,F,\theta} \right)\right\rVert_{\infty}\\
    &\leq \frac{1}{J}\sum_{j=1}^J \sum_{i=n-k}^{n}(1-\alpha^{(n-i)})G'(\theta)\\
    &\leq G'(\theta)\left(k-\frac{\alpha(1-\alpha^k)}{1-\alpha}\right),
\end{align*}
where the second-last line follows from Assumption \ref{assump:bounded-measurement}.
...
\begin{align*}
    ...
    &\leq \sum_{n=1}^N \E\left\lVert\nabla_\theta \Bigg(\left|\log\left(\sum_{j=1}^Jw_{n,j}^{F,\theta,1,k}\right)- \log\left(\sum_{j=1}^Jw_{n,j}^{F,\theta,\alpha,k}\right)\right|
    +\left|\log\left(\sum_{j=1}^Jw_{n-1,j}^{A, F,\theta,1,k}\right) + \log\left(\sum_{j=1}^Jw_{n-1,j}^{A, F,\theta,\alpha,k}\right)\right|\Bigg)\right\rVert_\infty \\
    &\leq 2NG'(\theta)\left(k-\frac{\alpha(1-\alpha^k)}{1-\alpha}\right) 
    \leq 2NG'(\theta)k.
\end{align*}

Now, the third term, $\E||\nabla_\theta\hat s_k^\alpha(\theta) -  \nabla_\theta\hat\ell^\alpha(\theta)||_\infty$, fortunately has already been dealt with. From the variance argument, we know that
$$||\nabla_\theta\hat s_k^\alpha(\theta) -  \nabla_\theta\hat\ell^\alpha(\theta)||_\infty \leq 2\frac{\alpha^k}{1-\alpha}NG'(\theta),$$
and taking expectations on both sides yields our desired result.

Therefore, our decomposition yields the bias bound
\begin{align*}
    &||\nabla_\theta\ell(\theta) - \E\nabla_\theta \hat\ell^\alpha(\theta)||_\infty \leq \E||\nabla_\theta\ell(\theta) - \nabla_\theta \hat\ell^\alpha(\theta)||_\infty \\
    &\leq \E||\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)||_\infty + \E||\nabla_\theta\hat s_k^1(\theta) - \nabla_\theta\hat s_k^\alpha(\theta)||_\infty + \E||\nabla_\theta\hat s_k^\alpha(\theta) -  \nabla_\theta\hat\ell^\alpha(\theta)||_\infty \\
    &\lesssim \min_{k \leq N} \left(N\frac{CG'(\theta)k}{\sqrt{J}} + NG'(\theta)(1-\epsilon)^{\floor{k/(c\log(J))}} + 2NG'(\theta)\left(k-\frac{\alpha(1-\alpha^k)}{1-\alpha}\right) + 2\frac{\alpha^k}{1-\alpha}NG'(\theta)\right) \\
    &\lesssim \min_{k \leq N} NG'(\theta)\left(\frac{k}{\sqrt{J}}+(1-\epsilon)^{\floor{k/(c\log(J))}}+k+\frac{\alpha^k  + \alpha^{k+1} - \alpha}{1-\alpha}\right).
\end{align*}

\subsection{Bounding the MSE of MOP-$\alpha$}

\kevin{First term sums up $k$ terms, $O(Nk^2)$ error, second term bounded by $k$ applications of the Markov kernel, so just apply forgetting of the particle filter result. }

We can obtain an $O(N)$ MSE bound from an argument that is similar to the bias argument. Simply observe that 
...
We see, by a similar argument to the bias bound, that
\begin{align*}
...
\kevin{Trying to bound MOP-(1,k) from MOP-alpha yields exactly the same result as above! Seems like I can't get rid of the k in the third term within the parentheses.}
% \begin{align*}
%     &\left\lVert\nabla_\theta\log\left(\sum_{j=1}^J w_{n,j}^{F,\theta,1,k}\right)-\nabla_\theta\log\left(\sum_{j=1}^J w_{n,j}^{F,\theta,\alpha}\right)\right\rVert_{2}^2\\
%     &\leq \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta \left(\sum_{i=n-k}^n\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right) - \sum_{i=1}^n{\alpha^{(n-i)}}\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)\right)\right\rVert_2^2 \\
%     &= \left\lVert\frac{1}{J}\sum_{j=1}^J \nabla_\theta \left(\sum_{i=n-k}^n\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right) - \sum_{i=n-k}^n{\alpha^{(n-i)}}\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right) - \sum_{i=1}^{n-k}{\alpha^{(n-i)}}\log\left(\frac{g_{i,j}^{A,F,\theta}}{g_{i,j}^{A,F,\phi}} \right)\right)\right\rVert_2^2 \\
%     &\leq \frac{1}{J}\sum_{j=1}^J \sum_{i=n-k}^{n}(1-\alpha^{(n-i)})\left\lVert\nabla_\theta\log\left(g_{i,j}^{A,F,\theta} \right)\right\rVert_2^2 + \frac{1}{J}\sum_{j=1}^J \sum_{i=1}^{n-k}\alpha^{(n-i)}\left\lVert\nabla_\theta\log\left(g_{i,j}^{A,F,\theta} \right)\right\rVert_2^2 \\
%     &\leq \frac{1}{J}\sum_{j=1}^J \sum_{i=n-k}^{n}(1-\alpha^{(n-i)})pG'(\theta)^2 + \frac{1}{J}\sum_{j=1}^J \sum_{i=1}^{n-k}\alpha^{(n-i)}pG'(\theta)^2\\
%     &\leq pG'(\theta)^2\left(k-\frac{\alpha(1-\alpha^k)+\alpha^k}{1-\alpha}\right),
% \end{align*}
...
\kevin{This is slightly alarming, in that the estimate really has $O(N^4)$ variance after accounting for summing over timesteps. In contrast, MOP-0 has only $O(N)$ variance after summing over timesteps. }
...
\kevin{Put some justification/make the setup of delmoral2011ci concrete here. Concentration of gradient estimate under particle measure to limit under posterior measure}
