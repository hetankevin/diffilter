...
\kevin{rewrite to de-emphasize dop as in presentation, sell mop as the big theoretical idea that explains AD through PFs and unifies two previous estimators}
...
\begin{prop}[Correctness of MOP-$\alpha$ (Algorithm \ref{alg:mop})]
    ...
    In particular, when $\theta$ is evaluated at $\phi$, the likelihood estimate agrees with the bootstrap filter. In this case, its gradient when $\alpha=1$ is the same as the estimator of \citet{doucet2011sf} when applied to the bootstrap filter,
    \begin{equation}
...
\kevin{Show MOP-1 is unbiased. Extend analysis to not-1 case. }
