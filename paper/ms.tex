...
The above densities are defined on a probability space $(\Omega, \Sigma, \prob).$ 
For the algorithmic interpretation of our theory, we identify the seed with an element of the sample space $\omega \in \Omega$. The random seed determines the sequence of pseudo-random numbers as generated by a computer, just as the outcome $\omega\in\Omega$ generates the sequence of random variables.
...
We avoid this issue by constructing a particle filtering algorithm called \textbf{Measurement Off-Parameter-$\alpha$} (MOP-$\alpha$) for which the resampling is invariant to $\omega$.
...
\begin{algorithm}[H]
    ...
    \label{alg:mop}
		...
		\textbf{For} $n=1,...,N$: \newline
            ...
            \hspace*{4mm} Resample particles ${X}_{n,j}^{F,\theta}={X}_{n,k_{j}}^{P,\theta}$. \newline
...
We obtain two coupled sets of particles, one under $\phi \in \Theta$, and another with the process model at $\theta \in \Theta$ but the resampling indices constructed from the first pass, with a ``baseline parameter'' $\phi$.
...
Consider the second stage of IFAD (Algorithm \ref{alg:ifad}) where one stops if $||\nabla_\theta \hat\ell^\alpha(\theta_n)|| \leq (1+\sigma) \epsilon$, where $\sigma \geq \frac{4 \Gamma}{(1-\beta)}$. Assume $-\ell$ is strongly convex and smooth, $\gamma I \preceq \nabla_\theta^2 (-\ell) \preceq \Gamma I$. Choose the learning rate $\eta$ and error tolerance $\epsilon$ such that $\eta \leq \frac{c(1-\beta)}{2\Gamma}$ for some $\beta \in (0,1)$. Then, for sufficiently large $\alpha$ and $J$ to ensure the score estimate is a $1/2$-approximation, the second stage of IFAD converges linearly to the MLE with probability at least $1-\delta$:
\vspace*{-2mm}
$$
...
\begin{algorithm}[H]
    ...
    \label{alg:ifad}
		...
		\hspace*{4mm} Obtain $g(\theta_m) = \nabla_{\theta_m} (-\hat\loglik(\theta_m))$, $H(\theta_m)$ s.t. $\lambda_{\min}(H) \geq c$. \newline
...
\vspace*{-1mm}
\begin{align*}
    d S&=(k \epsilon R^k+\delta(S-H)-\lambda(t) S) d t+d H-({\sigma S I}/{H}) d B_t, \\
    d I&=\left(\lambda(t) S-(m+\delta+\gamma) I\right) d t+({\sigma S I}/{H}) d B_t, \\
    d R^1&=(\gamma I-(k \epsilon+\delta) R^1) d t,..., 
    d R^k=(k \epsilon R^{k-1}\hspace{-3mm}-(k \epsilon+\delta) R^k) d t,
    \vspace*{-2mm}
\end{align*}
...
We show here that MOP-$\alpha$ targets the posterior and is strongly consistent for the likelihood when $\alpha=1$ or $\theta=\phi$. While the result is presented here as specific to MOP-$\alpha$, we actually prove a more general result in the supplementary material. That is, we show a strong law of large numbers for triangular arrays of particles with ``off-parameter'' resampling, where we resample the particles according to an arbitrary resampling rule not necessarily in proportion to the targeted distribution of interest, to weights that encode the cumulative discrepancy between the resampling and the target distribution instead of equal weights.
