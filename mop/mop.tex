...
\begin{algorithm}[t!]
  ...
  \For{$n$ in $\seq{1}{N}$}{
    ...
    \;
    Simulate for prediction:
    ${X}_{n,j}^{P,\theta}\sim {f}_{{X}_{n}|{X}_{n-1}}\big(\mydot|{X}_{n-1,j}^{F};{\phi}\big)$ for $j\ \mathrm{in}\ \seq{1}{J}$ \nllabel{dop-alpha:step1}
    \;
    Adjust weights: $\displaystyle w_{n,j}^{P,\theta} = w_{n,j}^{P,\theta} \times
    \frac{{f}_{{X}_{n}|{X}_{n-1}}\big({X}_{n,j}^{P,\theta}|{X}_{n-1,j}^{F};{\theta}\big)}{{f}_{{X}_{n}|{X}_{n-1}}\big({X}_{n,j}^{P,\theta}|{X}_{n-1,j}^{F};{\phi}\big)}$
    for $j\ \mathrm{in}\ \seq{1}{J}$
