...
The broad idea is that the first term can be controlled by a mixing argument, the second term garners $O(Nk^2)$ error, and the third term is $O\left(\frac{N}{1-\alpha}\right)$. It is unclear whether the bias can be reduced further -- the gradient is a sum of $N$ terms, and is therefore $O(N)$ itself. 

The first term -- bound ground truth of conditional scores from particle approximation conditional on correct filtering distribution at time $n-k$, bound that from particle approximation conditional on arbitrary filtering distribution at time $n-k$. First term sums up $k$ terms, $O(Nk^2)$ error, second term bounded by $k$ applications of the Markov kernel, so just apply forgetting of the particle filter result. 

But you can get an MSE bound like this! Simply decompose
$$||\nabla_\theta\ell_\theta - \nabla_\theta \hat\ell^\alpha(\theta)||_2^2 \leq ||\nabla_\theta\ell_\theta - \nabla_\theta\hat s_k^1(\theta)||_2^2 + ||\nabla_\theta\hat s_k^1(\theta) - \nabla_\theta\hat s_k^\alpha(\theta)||_2^2 + ||\nabla_\theta\hat s_k^\alpha(\theta) -  \nabla_\theta\hat\ell^\alpha(\theta)||_2^2,$$
and observe that 
$$||\nabla_\theta\ell_\theta - \nabla_\theta \hat\ell^\alpha(\theta)||_\infty =  \left\lVert\sum_{n=1}^N \nabla_\theta \hat\ell_n^\alpha(\theta) - \sum_{n=1}^N \nabla_\theta \ell_n(\theta)\right\rVert_\infty \leq \sum_{n=1}^N\left\lVert \nabla_\theta \hat\ell_n^\alpha(\theta) -  \nabla_\theta \ell_n(\theta)\right\rVert_\infty.$$
