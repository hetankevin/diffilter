...
\begin{lem}[Particle Marginals]
    ...
    Suppose that $\{(X_j^J,u_j^J),j=1,\dots,J\}$ targets $f_X$. Also suppose that $Z_j^J \sim f_{Z|X}(\cdot | X_j^J)$ where $f_{Z|X}$ is a conditional probability density function corresponding to a joint density $f_{X,Z}$ with marginal densities $f_X$ and $f_Z$. Then, if the $u_j^J$ are bounded, $\{(Z_j^J,u_j^J)\}$ targets $f_Z$.
\end{lem}
\begin{proof}

    We want to show that
     $$\frac{\sum_j h(Z_j^J) u_j^J}{\sum_j u_j^J} \stackrel{a.s.}{\to} \E_{f_Z}[h(Z)] = \E_{f_X}[\E_{f_{Z|X}}[h(Z) | X]].$$

    We know that by definition, for any measurable and bounded functional $g$ with domain $\gX$,
    $$\frac{\sum_j g(X_j^J)u_j^J}{\sum_j u_j^J} \stackrel{a.s.}{\to} \E_{f_X}[g(X)].$$

    Let $\bar{u}_j^J = \frac{J u_j^J}{\sum_j u_j^J}$. Examine the numerator and denominator of the quantity $$\frac{J^{-1}\sum_j h(Z_j^J) u_j^J}{J^{-1}\sum_j u_j^J} = \frac{J^{-1}\sum_j h(Z_j^J) \bar{u}_j^J}{J^{-1}\sum_j \bar{u}_j^J}.$$

    The denominator converges to $1$ almost surely. The numerator, on the other hand, is
    $$\frac{1}{J}\sum_j h(Z_j^J)\bar{u}_j^J,$$

    which by the same fourth moment argument to the above lemma converges almost surely to the limit of its expectation
    $$\lim_{J\to\infty} \E\left[\frac{1}{J}\sum_j h(Z_j^J)\bar{u}_j^J\right] = \lim_{J\to\infty} \E_{X_j^J}\left[\E_{Z_j^J|X_j^J}\left[\frac{1}{J}\sum_j  h(Z_j^J)\bar{u}_j^J\bigg|X_j^J\right]\right].$$

    It remains to swap the limit and expectations. We can do so with the bounded convergence theorem, and therefore obtain   
    $$\frac{1}{J}\sum_j h(Z_j^J)\bar{u}_j^J \stackrel{a.s.}{\to} \E_{f_Z}[h(Z)] = \E_{f_X}[\E_{f_{Z|X}}[h(Z) | X]].$$ 
