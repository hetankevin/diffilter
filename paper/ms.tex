...
However, iterated filtering algorithms have shown that this is practical in various scientific investigations, for example \cite{king08,blake14,pons-salort18,subramanian21,fox22,drake23}.
...
In short, instead of differentiating through a Monte Carlo algorithm directly, we instead use AD to differentiate through a series of measurement density ratios and a differentiable simulator.
...
Similar quantities are called {\it off-policy} in reinforcement learning, and a parameter in a POMP model is analogous to a policy in a corresponding control model \ed{Kevin, would it be good to add your favorite RL reference here? Also, you may want to edit this sentence. I think something like this will help the reader to ``get'' our terminology.}.
Specifically, MOP-$\alpha$ evaluates the likelihood at some $\theta \in \Theta$, but instead resamples the particles according to the indices generated by a vanilla particle filter run at some $\phi \in \Theta$.
...
\begin{figure}[t!]
    ...
    \caption{Convergence diagnostics for NUTS with the informative nonparametric empirical Bayes prior from applying KDE to preliminary searches using IF2. We display the results for the trend parameter in the Dhaka cholera model of \cite{king08}. The NUTS sampler mixes quickly, and the posterior estimates from each chain share roughly the same posterior mode.}
    \label{fig:nuts-eb}
\end{figure}
