...
The DOP-$\alpha$ and MOP-$\alpha$ procedures therefore provide us with a unifying framework for reasoning about smooth extensions of weighted particle filters that target the posterior and provide reasonable gradient estimates when differentiated. This fixed-seed reweighting hopefully alleviates some confusion that can arise from differentiating through a quantity yielded by ostensibly non-differentiable resampling. This is because in the former we differentiate through smooth likelihood ratios, and in the latter we differentiate through a differentiable simulator with the reparametrization trick and through smooth likelihood ratios in order to adjust for the resampling. 

This still leaves us with the question of designing a reasonably effective procedure for likelihood maximization with these new tools. 
...
\kevin{Do groups of ten and take the best one from each group. Omit old IF2 from figure in the paper, provide complete figure with old IF2 in the appendix, saying that we modified the settings because of more modern practice. }
