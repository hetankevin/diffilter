...
\kevin{Turn all theorems and lemmas into remarks, one big section on results we think may be true but leave to future work to prove. }

\kevin{Release arxiv version concurrently with an initial release of the package. Target April release, grant results for DMS come back then. }

\kevin{For a software paper, JOSS, signal that the software is careful. JSS is higher impact, but difficult to get a paper in. }

Observe that at Algorithm \ref{alg:mop}, at each timestep the corrected filtering importance weights $w_{n,j}^{F,\theta}$ optionally propagate to the next timestep for us to maintain the correction. We introduce a discounting parameter $\alpha$ that affects the weights, so $w_{n,j}^{P,\theta} := (w_{n-1,j}^{F,\theta})^{\alpha}$. Note that no matter what value $\alpha$ takes on, the system evolves according to $\phi$ when we perform the correction to evaluate the likelihood at $\theta$. The filter fully targets the posterior when $\theta=\phi$, or $\alpha=1$. As previously mentioned, the value of $\alpha$ therefore informs a bias-variance tradeoff. We elaborate on this below. 
...
\begin{conj}[Convergence of Gradient Methods]
...
\begin{conj}[Change of Particle Measure]
...
\begin{conj}[Particle Posteriors]
...
\begin{conj}[Likelihood Proper Weighting]
...
We present the doubly off-policy differentiable particle filter for completeness. 
...
In the event where a seed is fixed, all expressions in Algorithm \ref{alg:dop} can be considered to be conditional on $\omega \in \Omega$.  
...
Here, we consider an off-policy particle filter where \code{dmeasure} are computed at $\phi$ and we leverage the reparametrization trick to obtain derivatives with regard to the process model.
In the event where a seed is fixed, all expressions in Algorithm \ref{alg:mop} can be considered to be conditional on $\omega \in \Omega$.  
...
\kevin{I have no clue how to prove this. Someone, anyone, help!}
...
\begin{conj}[Consistency of MOP-1 Gradient Estimate]
...
\begin{proof}
    ...
    We know that the variance of the MOP-1 gradient estimate $\nabla_\theta \hat\ell_J(\theta) = \frac{1}{J}\sum_{j=1}^J \nabla_\theta w_{N,j}^{F,\theta}$ goes to zero coordinate-wise as $J\to\infty$ by a particle filter law of large numbers for bounded functionals of particles (\kevin{Prove general LLN for functionals of MOP under boundedness assumptions}), so it suffices to show that it is asymptotically unbiased.
    ...
    $$\lim_{J\to\infty} \E_{\hat\pi_{1:N,J}}[\hat\lik_J(\theta)] = \lik(\theta)$$
    uniformly for all $\theta \in \gN(\phi)$. 
    \kevin{Formalize this.}

    \kevin{Need to show that gradients converge almost surely uniformly (doesn't this follow directly from boundedness?) to swap limits, according to Terry Tao.}

    As the gradient is bounded uniformly for all $J$ and consistency for the likelihood estimate holds for all $\theta$ in an open neighborhood of $\phi$ (and therefore all $\theta'$ in a sufficiently small neighborhood of $\theta$), we can swap the derivative and limit by the bounded convergence theorem, and then the derivative and expectation by the very same, to find that 
    $$\nabla_\theta\lik(\theta) 
    = \nabla_\theta \lim_{J\to\infty} \E_{\hat\pi_{1:N,J}}[\hat\lik_J(\theta)] 
    = \lim_{J\to\infty} \nabla_\theta \E_{\hat\pi_{1:N,J}}[\hat\lik_J(\theta)] 
    = \lim_{J\to\infty} \E_{\hat\pi_{1:N,J}}[\nabla_\theta \hat\lik_J(\theta)].$$
...
\begin{conj}[Concentration of Measure for Gradient Estimate]
    \label{lemma:grad_bound}
    Assume $\text{osc} \left(\frac{\partial}{\partial_\theta} \hat{\ell}_\theta(x_{n,j}^P) \right) \leq 1$. For $||\nabla_\theta \hat{\loglik}_\theta(x_{n,j}^P) - \nabla_\theta \loglik_\theta(x^P)|| = ||\nabla_\theta \hat{\loglik}(\theta) - \nabla_\theta \loglik(\theta)||$ to be bounded by $\epsilon$ with probability $1-\delta$, we require
    \begin{equation}
...
Apply the Hoeffding-type concentration inequality from Del Moral and Rio \cite{delmoral2011ci} and another union bound to find that  
...
\begin{conj}[Minimum Eigenvalue Bound for Hessian Estimate]
