...
In the case of the Dhaka cholera model of \cite{king08}, when an aggressive geometric cooling multiplier of 0.95 and initial random walk standard deviation of 0.02 is used,  \ed{[initial convergence happens  BE MORE PRECISE] initial convergence to within xxx=40?? log-likelihood units of the maximum} occurs within 40 iterations. 
However, even when applying much larger amounts of Monte Carlo effort, IF2 fails to get within five log-likelihood units of the maximum on this problem (Figure~\ref{fig:boxplot}).
...
For simplicity, we only prove this for MOP-$1$ and note that the general case follows as the required $\epsilon$-approximation can still be obtained for sufficiently large $\alpha$. 
When $\alpha=1$, the required $J$ is given in \arxiv{Appendix~\ref{appendix:convergence}}{the supplementary information}. 
