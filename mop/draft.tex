...
There is an $\epsilon>0$ such that $\epsilon^{-1}>f_{Y_n \mid X_n}\left(y_n^* \mid x_n; \theta\right)>\epsilon$ and a $G'(\theta)>0$ such that $||\nabla_\theta \log f_{Y_n \mid X_n}\left(y_n^* \mid x_n; \theta\right)||_\infty < G'(\theta)$ for all $1 \leq n \leq N, x_n \in \gX$ and $\theta \in \Theta.$
\end{aspt}
...
Let $\mathcal{M}$ be an open subset of $\Omega$. There exists some function $G(\theta)$ and constants $G=\sup _{\theta \in \mathcal{N}} G(\theta)<\infty$, $H=\sup _{\theta \in \mathcal{N}} H(\theta)<\infty$ such that
    $$
    \|\nabla_\theta \ell(\theta, \phi, \omega, J)\|_\infty <G(\theta) \leq G<\infty, \;\;
    \|\nabla^2_\theta \ell(\theta, \phi, \omega, J)\|_\infty <G(\theta) \leq G<\infty
    $$
...
$$w_{n,j}^{P,\theta} = w_{n-1,\theta}^{F,\theta}\frac{g_{n,j}^\theta}{g_{n,j}^\phi} = \prod_{i=1}^n \frac{g_{i,j}^{A,\theta}}{g_{i,j}^{A,\phi}},$$
where we use the $(\cdot)^A$ superscript to denote the ancestral trajectory of the $j$-th particle at timestep $n$.

Using the log-derivative identity again,
$$\frac{\nabla_\theta w_{n,j}^{P,\theta}}{w_{n,j}^{P,\theta}} = \nabla_\theta \log w_{n,j}^{P,\theta} = \nabla_\theta \log \left(\prod_{i=1}^n \frac{g_{i,j}^{A,\theta}}{g_{i,j}^{A,\phi}}\right) = \nabla_\theta \sum_{i=1}^n \left(\log g_{i,j}^{A,\theta} - \log g_{i,j}^{A,\phi}\right) = \sum_{i=1}^n \nabla_\theta \log g_{i,j}^{A,\theta}.$$

So 
$$  \nabla_\theta \sum_{n=1}^N \log g_{n,j}^{A,\theta} = \nabla_\theta \log\left(\prod_{n=1}^N g_{n,j}^{A,\theta}\right) =  \nabla_\theta \log\left(\prod_{n=1}^N f_{Y_n|X_n}\left(y_n^* | x_{n,j}^{A, P,\theta}\right)\right) = \nabla_\theta \log f_{Y_{1:N}|X_{1:N}}\left(y_{1:N}^* | x_{1:n,j}^{A, P,\theta}\right),$$

and 
$$\nabla_\theta w_{N,j}^{P,\theta} = w_{N,j}^{P,\theta} \sum_{n=1}^N \nabla_\theta \log g_{n,j}^{A,\theta} = w_{N,j}^{P,\theta} \nabla_\theta \log f_{Y_{1:N}|X_{1:N}}\left(y_{1:N}^* | x_{1:n,j}^{A, P,\theta}\right).$$
...
\section{Analysis of MOP-$\alpha$ Variance}


We first begin with a lemma based on the work of \cite{yu94}. \cite{kausik23} simplify this to the special case of Markov chains. We further adapt this to our setting below.
\begin{lem}
    Let $h: \gX^N \to \R^n$ be a bounded measurable function with bound $C$ on trajectories of points in the state space $\gX$ (by considering the domain $\gX^N$, this also encompasses functions of segments of trajectories). Further assume that the underlying Markov chain $(X_n)_{n=1}^N$ has mixing time $t_{\text {mix}}=t_{\text{mix}}\left(\frac{1}{4}\right)$. Consider the joint distribution $\chi$ over the product of the $\sigma$-algebras of $S$ such segments separated by $s$ timesteps, with marginals $\chi_i$. Let the product distribution of the marginals $\chi_i$ be called $\Xi$. Then, we have
$$
\left|\mathbb{E}_\chi h-\mathbb{E}_{\Xi} h\right| \leq \frac{4 C(S-1)}{4^{s\cdot t_{\text{mix}}}}.
$$


\end{lem}
\begin{proof}
    Remember that each of our Markov processes is mixing, so there exists $t_{m i x, j}=t_{m i x, j}\left(\frac{1}{4}\right)$ and a stationary distribution $d_j$ so that $T V\left(P_j^n(x, \cdot), d_j\right)<\frac{1}{4}$ for $n \geq t_{m i x, j}$. Let $t_{m i x}=\max _j t_{m i x, j}$. Since the decay in total variation distance is multiplicative, $T V\left(P_j^n(x, \cdot), d_j\right)<\left(\frac{1}{4}\right)^c$ for all $j$ and $n \geq c t_{m i x}$. This implies that
    $$
    \max _j T V\left(P_j^n(x, \cdot), d_j\right)<\left(\frac{1}{4}\right)^{\frac{T_n}{4 t_{m i x}}-1}=4 \lambda^n
    $$
    where $\lambda=\left(\frac{1}{4}\right)^{\frac{1}{t_{m i x}}}$.
\end{proof}

It is known from \cite{poyiadjis11} that the variance of MOP-1 is no smaller than $O(N^2)$, with an analogous upper bound on the MSE. We now address the case of $\alpha<1$.

We can decompose the MOP-$\alpha$ estimator as follows. When $\theta=\phi$,
$$\hat{\mathcal{L}}(\theta):=\prod_{n=1}^N L_n^{A, \theta, \alpha}=\prod_{n=1}^N L_n^\phi \cdot \frac{\sum_{j=1}^J w_{n, j}^{F, \theta}}{\sum_{j=1}^J w_{n, j}^{P, \phi}}=\prod_{n=1}^N L_n^{A, \theta, \alpha}=\prod_{n=1}^N L_n^\phi \cdot \frac{\sum_{j=1}^J w_{n, j}^{F, \theta}}{\sum_{j=1}^J (w_{n-1, j}^{F, \phi})^\alpha}.$$
...
\begin{lem}[Concentration of Measure for Gradient Estimate]
    ...
    \begin{align}
    J > \max\left\{2NG'(\theta)\frac{r_N\sqrt{p}}{\epsilon}\left(1+h^{-1}\left(\log\left(\frac{2p}{\delta}\right)\right)\right), 8N^2G'(\theta)^2\beta_N^2\frac{p\log(2p/\delta)}{\epsilon^2}\right\},
    \end{align}
    where $G'(\theta)$ is defined as in Assumption \ref{assump:bounded-measurement}, $h(t) = \frac{1}{2}(t - \log(1+t))$, and $\beta_N$ and $r_N$ are two additional finite model-specific constants that do not depend on $J$, but do depend on $N$, as defined in \cite{delmoral2011ci}. 

    Equivalently, with probability at least $1-\delta$, it holds that
    \begin{align}
        ||\nabla_\theta \hat\ell(\theta) - \nabla_\theta \ell(\theta)||_2 \leq G'(\theta)N\left(\frac{r_N\sqrt{p}}{J}(1+h^{-1}(\log(2p/\delta))) + \sqrt{\frac{2p\log(2p/\delta)}{J}}\beta_N\right).
    \end{align}
\end{lem}

\paragraph{Remark:} According to \cite{delmoral2011ci}, under some regularity conditions, $r_N$ and $\beta_N$ are linear in the trajectory length $N$. Additionally, if the system is stationary enough, $NG'(\theta)$ roughly scales with $G(\theta)$, so one can consider the error to be roughly linear in the trajectory length, $O(G(\theta)N) \approx O(G'(\theta)N^2)$, if all these conditions are fulfilled. This corresponds to the finding by \cite{poyiadjis11} that the variance of the estimate is quadratic in the trajectory length.
...
with probability at least $1-2Np\exp(-t)$. It therefore holds that with the same probability, that

\begin{align}
    \left\lVert\frac{1}{J}\sum_{j=1}^J\sum_{n=1}^N\nabla_\theta \log g_{n,k_j}^{A,\theta} - \nabla_\theta \ell(\theta) \right\rVert_{\infty} \leq G'(\theta)N\left(\frac{r_N}{J}(1+h^{-1}(t)) + \sqrt{\frac{2t}{J}}\beta_N \right)
\end{align}
